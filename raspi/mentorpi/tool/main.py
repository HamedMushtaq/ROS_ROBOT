...
class MainWindow(QWidget, ui.Ui_Form):
    def __init__(self):
        ...
        self.machine = None
        self.version = None
        self.voice_language = None
        self.get_typerc()
        self.domain_old = None
        self.domain = self.get_domian()
        ...
        depth_camera_list = ['ascamera','usb_cam','aurora']
        lidar_list = ['LD19', 'MS200']
        ...
        self.comboBox_language.setCurrentIndex(language_list.index(self.voice_language))
    ...
    def change_language(self, name):
        if self.radioButton_zn.isChecked() and name.text() == '中文':
            ...
            self.label_language.setText('语音功能')
            ...
            self.label_language.setText('Voice Language')
    ...
    def get_domian(self):
        try:
            result = subprocess.run(
                ["docker", "exec", "MentorPi", "cat", "/home/ubuntu/ros2_ws/.typerc"],
                capture_output=True,
                text=True,
                check=True
            )
            content = result.stdout.strip()
            domain = re.findall(r'export ROS_DOMAIN_ID.*?\n', content)[0].split('=')[1].replace('\n', '')
    ...
    def get_typerc(self):
        with open(self.typerc_path, "r") as f:
            ...
            self.voice_language = re.findall(r'export ASR_LANGUAGE.*?\n', data)[0].split('=')[1].replace('\n', '')
            self.domain_old = re.findall(r'export ROS_DOMAIN_ID.*?\n', data)[0].split('=')[1].replace('\n', '')
    ...
    def set_typerc(self):
        with open(self.typerc_path, "r") as f:
            ...
            data = re.sub(r'export DEPTH_CAMERA_TYPE=.*', f'export DEPTH_CAMERA_TYPE={self.comboBox_depth_camera.currentText()}', data)
            data = re.sub(r'export LIDAR_TYPE=.*', f'export LIDAR_TYPE={self.comboBox_lidar.currentText()}', data)
            data = re.sub(r'export MACHINE_TYPE=.*', f'export MACHINE_TYPE={self.comboBox_machine.currentText()}', data)
            data = re.sub(r'export ASR_LANGUAGE=.*', f'export ASR_LANGUAGE={self.comboBox_language.currentText()}', data)
