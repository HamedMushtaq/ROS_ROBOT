...
class PacketFunction(enum.IntEnum):
    # Control functions that can be implemented via serial port
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1               # LED control
    PACKET_FUNC_BUZZER = 2            # Buzzer control
    PACKET_FUNC_MOTOR = 3             # Motor control
    PACKET_FUNC_PWM_SERVO = 4         # PWM servo control, board from inside to outside依次为1-4
    ...
    PACKET_FUNC_SBUS = 9              # Model Airplane Remote Control Acquisition
...
class Board:
    ...
    def get_button(self):
    # Get the button status and return the button ID (1 represents button 1, 2 represents button 2) and status (0 represents release, 1 represents press, 2 represents long press).
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # Obtain IMU data, return ax, ay, az, gx, gy, gz
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # Sub-command is 0x01 to set SSID, the second byte is the string length, which includes the '\0' string terminator 
